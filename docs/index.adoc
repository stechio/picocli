...
:revnumber: 3.0.0-alpha1-SNAPSHOT
:revdate: 2017-12-21
:toc: left
:numbered:
:toclevels: 2
...
If the field is `null`, picocli will instantiate it. If the `Collection` type is not a concrete class, picocli will
make a best effort to instantiate it based on the field type: `List -> ArrayList`, `OrderedSet -> TreeSet`,
...
----
If the field is `null`, picocli will instantiate it. If the type is not a concrete class, picocli will
instantiate a `LinkedHashMap` to preserve the input ordering.
...
Picocli v0.9.8 introduced two new option attributes, `versionHelp` and `usageHelp`, and from v2.0, the `help` attribute is deprecated.
...
Since v0.9.8, the `CommandLine` class offers two methods that allow external components to detect whether
...
[source,java]
----
Object result = CommandLine.call(new MyCallable(), System.err, args);
----
...
[source,java]
----
CommandLine.run(new MyRunnable(), System.err, args);
----
...
* otherwise, if the command line arguments were parsed successfully, let the specified `IParseResultHandler` handle the parse result

Picocli provides some default `IParseResultHandler` implementations for common tasks:
...
[source,groovy]
----
@Grab('info.picocli:picocli:3.0.0-alpha1-SNAPSHOT')
...
so be aware that you will have to write `arrayOf(...)` for the `names`, `description` and `type` attributes.

(Kotlin https://blog.jetbrains.com/kotlin/2017/06/early-access-program-for-kotlin-1-2-has-been-started/[1.2 may support] array literals like `names = ["-h", "--help"]` in annotations.)
...
=== Gradle
----
compile 'info.picocli:picocli:3.0.0-alpha1-SNAPSHOT'
----
...
=== Scala SBT
----
libraryDependencies += "info.picocli" % "picocli" % "3.0.0-alpha1-SNAPSHOT"
----

=== Ivy

----
<dependency org="info.picocli" name="picocli" rev="3.0.0-alpha1-SNAPSHOT" />
----
